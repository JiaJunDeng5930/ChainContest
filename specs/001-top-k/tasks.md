...
- [X] T030 [P] [US3] 在 `frontend/src/components/Leaderboard.tsx` 展示 Top-K 榜单、排名变更与奖池份额
- [X] T031 [P] [US3] 在 `frontend/src/components/AdminActions.tsx` 提供冻结/结算/封榜按钮（任何用户可见），含重试与失败提示
- [X] T032 [US3] 在 `frontend/src/lib/contest/payout.ts` 实现领奖与退出交易封装，处理重复调用幂等响应
