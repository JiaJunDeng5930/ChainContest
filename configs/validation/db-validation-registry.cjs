...
const actorContextSchema = z
  .object({
    ...
    source: z.enum(walletSourceValues),
...
const contestItemSelectorSchema = z
  ...
  .refine(
    (value) => !value.chainId || supportedChainIds.includes(value.chainId),
...
const contestFilterSchema = z
  .object({
    chainIds: z
      .array(z.number().int().refine((id) => supportedChainIds.includes(id), { message: 'Unsupported chain id in filter' }))
      .nonempty()
      .optional(),
...
const userContestFilterSchema = z
  .object({
    chainIds: z
      .array(z.number().int().refine((id) => supportedChainIds.includes(id)))
      .optional(),
    statuses: z.array(z.string().min(1)).optional(),
    timeRange: z
      .object({
...
const contestDomainWriteSchema = z.discriminatedUnion('action', [
  z.object({
    ...
    payload: z.object({
      ...
      originTag: z.enum(contestOriginValues).optional(),
      timeWindow: z.object({
...
const registry = [
  ...
  },
  {
    ...
    rule: {
      ...
      schema: userWalletMutationSchema
    }
  },
  {
    ...
    rule: {
      ...
      schema: contestQueryRequestSchema
    }
  },
  {
    ...
    rule: {
      ...
      schema: userContestQueryRequestSchema
    }
  },
  {
    ...
    rule: {
      ...
      schema: contestDomainWriteSchema
    }
  },
  {
