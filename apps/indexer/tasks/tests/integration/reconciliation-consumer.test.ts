import { beforeEach, describe, expect, it, vi } from 'vitest';
import type { Job } from 'pg-boss';
import type { TasksApplication } from '@indexer-tasks/bootstrap/app';
import {
  registerReconciliationWorker,
  type ReconciliationWorkerDependencies
} from '@indexer-tasks/queue/workers/reconciliationWorker';
import {
  type ReconciliationPayload,
  type ReconciliationProcessor,
  ReportAlreadyProcessedError
} from '@indexer-tasks/services/reconciliationProcessor';

const createJob = (overrides: Partial<Job<unknown>> = {}): Job<unknown> => ({
  id: overrides.id ?? 'reconcile-1',
  name: overrides.name ?? 'indexer.reconcile',
  data:
    overrides.data ?? {
      reportId: 'report-1',
      contestId: 'contest-a',
      chainId: 11155111,
      rangeFromBlock: '1000',
      rangeToBlock: '2000',
      generatedAt: new Date('2025-10-21T12:00:00Z').toISOString(),
      differences: [],
      notifications: [{ channel: 'ops', target: 'oncall' }]
    },
  priority: overrides.priority ?? 0,
  state: overrides.state ?? 'created',
  retrycount: overrides.retrycount ?? 0,
  retrylimit: overrides.retrylimit ?? 3,
  createdon: overrides.createdon ?? new Date('2025-10-22T00:00:00Z').toISOString(),
  nextiteration: overrides.nextiteration ?? null,
  singletonKey: overrides.singletonKey ?? 'singleton::reconcile::contest-a',
  startedon: overrides.startedon,
  completedon: overrides.completedon,
  keepuntil: overrides.keepuntil
});

describe('reconciliation queue consumer', () => {
  let app: TasksApplication;
  let processor: ReconciliationProcessor;
  let dependencies: ReconciliationWorkerDependencies;
  let registeredHandler: ((job: Job<unknown>) => Promise<void>) | null;
  let parsedPayload: ReconciliationPayload;

  beforeEach(() => {
    registeredHandler = null;

    parsedPayload = {
      reportId: 'report-1',
      contestId: 'contest-a',
      chainId: 11155111,
      rangeFromBlock: '1000',
      rangeToBlock: '2000',
      generatedAt: new Date('2025-10-21T12:00:00Z'),
      differences: [],
      notifications: [{ channel: 'ops', target: 'oncall' }],
      metadata: {
        replay: false
      }
    } satisfies ReconciliationPayload;

    processor = {
      process: vi.fn().mockResolvedValue({ status: 'processed', notificationsDispatched: 1 })
    } as unknown as ReconciliationProcessor;

    dependencies = {
      processor,
      parsePayload: vi.fn(() => ({ payload: parsedPayload, idempotencyKey: `report:${parsedPayload.reportId}` }))
    } satisfies ReconciliationWorkerDependencies;

    app = {
      config: {
        queue: { concurrency: 2 }
      } as unknown as TasksApplication['config'],
      logger: {
        info: vi.fn(),
        warn: vi.fn(),
        error: vi.fn(),
        child: vi.fn().mockReturnThis()
      } as unknown as TasksApplication['logger'],
      metrics: {
        jobCounter: { labels: vi.fn().mockReturnThis(), inc: vi.fn() },
        jobDuration: { labels: vi.fn().mockReturnThis(), observe: vi.fn() },
        jobRetryCounter: { labels: vi.fn().mockReturnThis(), inc: vi.fn() },
        queueDepthGauge: { labels: vi.fn().mockReturnThis(), set: vi.fn() },
        lastSuccessGauge: { labels: vi.fn().mockReturnThis(), set: vi.fn() }
      } as unknown as TasksApplication['metrics'],
      http: {} as TasksApplication['http'],
      start: vi.fn(),
      stop: vi.fn(),
      isRunning: vi.fn(),
      publishJob: vi.fn(),
      registerWorker: vi.fn(async (_queue, handler) => {
        registeredHandler = handler as (job: Job<unknown>) => Promise<void>;
      })
    } as unknown as TasksApplication;
  });

  it('processes a reconciliation job successfully', async () => {
    await registerReconciliationWorker(app, dependencies);

    expect(app.registerWorker).toHaveBeenCalledWith(
      'indexer.reconcile',
      expect.any(Function),
      expect.objectContaining({ includeMetadata: true })
    );

    const job = createJob();
    const handler = registeredHandler;
    expect(handler).not.toBeNull();

    await handler!(job);

    expect(dependencies.parsePayload).toHaveBeenCalledWith(job.data);
    expect(processor.process).toHaveBeenCalledTimes(1);
    expect(processor.process).toHaveBeenCalledWith({
      envelope: expect.objectContaining({ jobId: job.id, queueName: job.name }),
      payload: parsedPayload,
      idempotencyKey: `report:${parsedPayload.reportId}`
    });
  });

  it('skips already processed reconciliation reports', async () => {
    await registerReconciliationWorker(app, dependencies);
    const handler = registeredHandler;
    expect(handler).not.toBeNull();

    const job = createJob({ id: 'reconcile-duplicate', retrycount: 1 });

    const processMock = processor.process as unknown as ReturnType<typeof vi.fn>;
    processMock.mockRejectedValueOnce(new ReportAlreadyProcessedError('report already handled'));

    await expect(handler!(job)).resolves.toBeUndefined();
    expect(processMock).toHaveBeenCalledTimes(1);
    expect(app.logger.warn).toHaveBeenCalledWith(expect.any(Object), 'duplicate reconciliation report skipped');
  });
});
