...
describe('POST /api/contests/[contestId]/price-source/update', () => {
  ...
  it('reads price source from on-chain config when metadata is missing', async () => {
    const buildContestDefinition = vi.fn().mockResolvedValue({
      contest: {
        contestId: 'contest-xyz',
        chainId: 31337,
      ...
      return {
        ...
        createPublicClient: vi.fn(() => ({
          readContract: vi.fn().mockResolvedValue({
            priceSource: '0x00000000000000000000000000000000000000ab'
          })
        }))
      };
    });
    ...
    expect(response.status).toBe(200);
    const body = (await response.json()) as { transaction: { to: string; data: string; value: string } };
    expect(body.transaction.to).toBe('0x00000000000000000000000000000000000000ab');
  });
});
